...
function VehicleSeating.AddSeat(seat, enterCallback, exitCallback)
	...
	script.StartCar:GetPropertyChangedSignal("Value"):connect(function()
		if script.StartCar.Value == true then
			if seat then
				if not seat:FindFirstChild("SeatWeld") or carjackingEnabled(vehicleObj) then
