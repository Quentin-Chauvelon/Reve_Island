...
local MaxSpeed = Vehicle:GetAttribute("MaxSpeed")
...
function Effects:OnHeartbeat(dt)
	...
	if EngineSoundEnabled then
		...
		local VehicleSeat = Vehicle:WaitForChild("Chassis"):WaitForChild("VehicleSeat") -- get the driver's seat
		
		if VehicleSeat and VehicleSeat.Occupant and  VehicleSeat.Occupant.Parent and Players:FindFirstChild(VehicleSeat.Occupant.Parent.Name) then -- if player can be found using the seat's occupant
			local PlayerGui = Players:FindFirstChild(VehicleSeat.Occupant.Parent.Name).PlayerGui.RaceTrack.Racing -- gt the player race track gui
			
			if PlayerGui then -- if the gui was found
				PlayerGui.Tachometer.Frame.RPM.Text = math.floor(math.round(enginePower) / 1000) -- get the rpm and change the rpm gui
				PlayerGui.Tachometer.Needle.Rotation = ((enginePower - 4000) / (MAX_RPM - 4000)) * 90 -- change the needle rotation based on the rpm
				PlayerGui.Speedometer.Frame.Speed.Text = math.round(VehicleSeat.AssemblyLinearVelocity.Magnitude) -- get the speed and change the speed gui
				PlayerGui.Speedometer.Needle.Rotation = (VehicleSeat.AssemblyLinearVelocity.Magnitude / MaxSpeed) * (-90) -- change the needle rotation based on the speed
			end
		end
