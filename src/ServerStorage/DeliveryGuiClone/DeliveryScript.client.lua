	...
	if not toggledebounce then
		...
		if ToggleHouseGUI.GuiOn.Value == true then -- if the gui is on, turn it off
			...
			script.Parent.Frame:TweenPosition(UDim2.new(1.01,0,0.98,0, nil, nil, 0.5))
			 if DeliveryGui.Visible == true then DeliveryGui:TweenPosition(UDim2.new(0.5,0,0.5,0), nil, nil, 0.5) end -- move the delivery gui to the left if it is visible
			wait(0.5)
			
		else -- if it's off, turn it on
			ToggleHouseGUI.GuiOn.Value = true
			if DeliveryGui.Visible == false then script.Parent.Frame:TweenPosition(UDim2.new(0.747,0,0.98,0, nil, nil, 0.5)) end
			script.Parent.Frame:TweenPosition(UDim2.new(0.747,0,0.98,0, nil, nil, 0.5))
			ToggleHouseGUI.TextLabel.Text = "" -- remove the cross ("X")
			 if DeliveryGui.Visible == true then DeliveryGui:TweenPosition(UDim2.new(0.37,0,0.5,0), nil, nil, 0.5) end -- move the delivery gui back to the center if it is invisible
			wait(0.5)
		end
