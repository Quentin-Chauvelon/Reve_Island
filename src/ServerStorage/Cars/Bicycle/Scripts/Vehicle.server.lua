...
local function onEnterSeat(obj, seat)
	...
	if seat and seat == DriverSeat then	
		-- Delete the player's car if he already has one
		if workspace.Cars:FindFirstChild(obj.Name) and workspace.Cars[obj.Name] ~= TopModel then
			workspace.Cars[obj.Name]:Destroy()
		end

		-- If the cars hasn't been taken yet, change its parent to the cars folder
		if TopModel.Parent ~= workspace.Cars then
			TopModel.Parent = workspace.Cars
		end

		-- If the driver is not the last player to have driven the car, change the car name to the player driving it now
		if TopModel.Name ~= obj.Name then
